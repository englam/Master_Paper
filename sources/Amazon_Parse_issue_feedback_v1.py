...
class Amazon_Parse(object):
    ...
    def amazon_star_clicked(self,url,query,test_file,selected_priority,amazon_robot_img,number_log):
        try:
            self.driver.get(url)
            ...
            self.available_date = lambda tag: tag.findAll("tr")[6].find("td").text if int(check_section1_detail_tr_len) > 6 else No_Date
        ...
        if Check_Initial:
            Filterted_priority_ok = "[" + str(datetime.now()) + "]" + " Filterted Priority: " + selected_priority + ", Initial OK"
            print(Filterted_priority_ok)

            ''' Save log to log_all'''
            Amazon_Parse.write_log(self, log_all, Filterted_priority_ok)

            ''' Save log to number_log'''
            Amazon_Parse.write_log(self, number_log, Filterted_priority_ok)

            Amazon_Parse.get_all_issues(self,query,test_file,amazon_robot_img,selected_priority,number_log)
        else:
            Filterted_priority_fail = "[" + str(datetime.now()) + "]" + " Filterted Priority: " + selected_priority + ", Initial Fail"
            ''' Save log to log_all'''
            Amazon_Parse.write_log(self, log_all, Filterted_priority_fail)

            ''' Save log to number_log'''
            Amazon_Parse.write_log(self, number_log, Filterted_priority_fail)

            self.wireless_type  = No_Value
            self.brand          = No_Value
            self.model_name     = No_Value
            self.available_date = No_Date
            self.price          = No_Value
            self.title          = No_Value
    ...
    def save_to_db(self,product_name,bug_url,number_log):
        ...
        try:
            ...
            ''' Save log to number_log'''
            Amazon_Parse.write_log(self, number_log, db_success_log)
            ...
            for i in bsobj.find("", {"id": "cm_cr-review_list"}).find("", {"class": "a-section review"}).next_siblings:
                ...
                if i.find("", {"class": "a-size-base a-link-normal review-title a-color-base a-text-bold"}) is not None:
                    ...
                    ''' Save log to number_log'''
                    Amazon_Parse.write_log(self, number_log, db_success_log_2)
            ...
            ''' Save log to number_log'''
            Amazon_Parse.write_log(self, number_log, product_save_db_error)
            Amazon_Parse.write_log(self, number_log, product_error_url)
    ...
    def get_amazon_ai_image(self,img_path, url,number_log):
        time.sleep(3)
        ...
        if a_string in self.driver.page_source:
            ...
            ''' Save log to number_log'''
            Amazon_Parse.write_log(self, number_log, Captcha_get)
            ...
            ''' Save log to number_log'''
            Amazon_Parse.write_log(self, number_log, Captcha_text_log)

            self.driver.find_element_by_id('captchacharacters').click()
            ...
            ''' Save log to log_captcha'''
            Amazon_Parse.write_log(self, number_log, robot_url)

        time.sleep(3)


    def get_all_issues(self,product_name,test_file,amazon_robot_img,selected_priority,number_log):
        ...
        ''' Save log to number_log'''
        Amazon_Parse.write_log(self, number_log, total_bugs_log)

        ''' Save log to number_log'''
        Amazon_Parse.write_log(self, number_log, current_url_log)
        ...
        for i in range(1, total_bugs + 1):
            ...
            ''' Save log to number_log'''
            Amazon_Parse.write_log(self, number_log, bugs_url_log)
            ...
            self.get_amazon_ai_image(amazon_robot_img, bugs_url,number_log)

            time.sleep(waiting_time_page)
            feedback_id = self.save_html_to_file(test_file)
            self.Click_Feedback_button(feedback_id,product_name)
            self.save_to_db(product_name,bugs_url,number_log)
...
def Start_Parse(querys,test_file,_browser,amazon_robot_img):
    ...
    for query in querys:
        ...
        for priority in priorities:
            ...
            #normal
            #amazon_query.amazon_star_clicked(amazon_query.start_search(url,query))

            amazon_query.amazon_star_clicked(search_query,query,test_file,selected_priority=priority,amazon_robot_img=amazon_robot_img)

            # 過濾三顆星Issues
            #amazon_query.amazon_star_filtered('3 star only')
            #amazon_query.get_all_issues('3',query,test_file)


            # 過濾二顆星Issues
            #amazon_query.amazon_star_filtered('2 star only')
            #amazon_query.get_all_issues('2',query,test_file)


            # 過濾一顆星Issues
            #amazon_query.amazon_star_filtered('1 star only')
            #amazon_query.get_all_issues('1',query,test_file)

            # 過濾Amazon 認定 Critical Issues
            #amazon_query.amazon_star_filtered('All critical')
            #amazon_query.get_all_issues('critical',query,test_file)
            ...
            #except:
            #write_log("Error_Log", "[" + str(datetime.now()) + "]" + " URL Failed - " + url)
            #amazon_query.close_web()
...
if __name__ == '__main__':
    ...
    keyword_1 = ["NETGEAR R9000","NETGEAR Orbi","NETGEAR WN3000RP","NETGEAR WNR2200","NETGEAR R6100","NETGEAR EX6400","NETGEAR WNDA4100"]
    keyword_2 = ["NETGEAR R7500v2","NETGEAR EX7300","NETGEAR WNDR4500","NETGEAR A6200v2","NETGEAR WNR2000","NETGEAR WNDR4700","NETWORK 300R","NETWORK N150R"]

    p1 = multiprocessing.Process(target=Start_Parse, args=(keyword_1,"test1",'firefox_proxy','test1.jpg',))
    p2 = multiprocessing.Process(target=Start_Parse, args=(keyword_2,"test2",'firefox_proxy','test2.jpg',))
    p1.start()
    time.sleep(1)
    p2.start()
